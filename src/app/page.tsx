...
import { loadTeams, saveTeams, loadSelectedTeamId, saveSelectedTeamId } from '@/lib/storage';
...
export default function HomePage() {
  ...
  const renameTeam = () => {
    ...
    if (!name) return;
    setTeams(prev =>
      prev.map(team => (team.id === selectedTeamId ? { ...team, name } : team))
    );
  };
  ...
  const getAvgExp = (): number => {
    ...
    return Math.round(list.reduce((sum, p) => sum + p.base_experience, 0) / list.length);
  };

  return (
    <main className="p-6 max-w-5xl mx-auto bg-yellow-100 min-h-screen text-gray-800 font-sans">
      <h1 className="text-4xl font-bold mb-6 text-center text-red-600">🎮 Pokémon Team Builder</h1>
      ...
      <SearchBar value={search} onChange={setSearch} notFound={!!search && !searchResult} />
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          ...
          {selectedTeam?.pokemons.length ? (
            <TeamList pokemons={selectedTeam.pokemons} onRemove={removeFromTeam} />
          ) : (
            <p>No Pokémon added yet.</p>
          )}
        </div>

        <TeamOverview types={getUniqueTypes()} avgExp={getAvgExp()} />
      </div>
    </main>
  );
}
